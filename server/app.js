...
mongoose.connect(dbMongo,function(err,res){
    if (err) {
        console.log(`Error al conectarse a la bd ${err}`); //Para identificar cuál es el error
    } else {
...
app.post('/api/setWord',function(req,res){
    let word = new Word(); //Variable local
...
function randomWord(callback){
    Word.find({}, function(err, words){ //Llaves vacías es que traiga todo
